// Auto-generated feature route: env-compliance-multimodal
// Domain: environmental compliance agent — Capability: producing sensor-driven compliance reports
// Mounted under: /api/env-compliance-multimodal
...
// POST /api/env-compliance-multimodal/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a environmental compliance agent. Your task is producing sensor-driven compliance reports. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'env-compliance-multimodal',
      input: payload,
      output: parsed,
...
// GET /api/env-compliance-multimodal/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/env-compliance-multimodal/:id — fetch one
...
// DELETE /api/env-compliance-multimodal/:id — discard a run
