// Auto-generated feature route: equipment-maintenance-autonomous
// Domain: telematics maintenance scheduler — Capability: scheduling from telematics + vibration data
// Mounted under: /api/equipment-maintenance-autonomous
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Mining Operations Optimizer'
      }
    };
...
// POST /api/equipment-maintenance-autonomous/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a telematics maintenance scheduler. Your task is scheduling from telematics + vibration data. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'equipment-maintenance-autonomous',
      input: payload,
      output: parsed,
...
// GET /api/equipment-maintenance-autonomous/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/equipment-maintenance-autonomous/:id — fetch one
...
// DELETE /api/equipment-maintenance-autonomous/:id — discard a run
