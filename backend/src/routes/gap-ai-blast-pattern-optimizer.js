...
// Route: /api/gap-ai-blast-pattern-optimizer
// Feature: Ai Blast Pattern Optimizer
// Description: (drill-and-blast design)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Blast Pattern Optimizer. (drill-and-blast design)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Blast Pattern Optimizer\nDescription: (drill-and-blast design)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-blast-pattern-optimizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-blast-pattern-optimizer', title: 'Ai Blast Pattern Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-blast-pattern-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-blast-pattern-optimizer']);
      return res.json({ slug: 'ai-blast-pattern-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-blast-pattern-optimizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-blast-pattern-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
