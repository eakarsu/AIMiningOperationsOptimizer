...
// Route: /api/gap-ai-commodity-price-impact
// Feature: Ai Commodity Price Impact
// Description: (production vs. market timing)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Commodity Price Impact. (production vs. market timing)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Commodity Price Impact\nDescription: (production vs. market timing)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-commodity-price-impact', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-commodity-price-impact', title: 'Ai Commodity Price Impact', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-commodity-price-impact', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-commodity-price-impact']);
      return res.json({ slug: 'ai-commodity-price-impact', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-commodity-price-impact', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-commodity-price-impact', items: [], error: (e && e.message) || 'Server error' });
  }
});
