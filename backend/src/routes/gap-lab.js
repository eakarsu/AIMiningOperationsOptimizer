...
// Route: /api/gap-lab
// Feature: Lab
// Description: Lab assay LIMS integration
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Lab. Lab assay LIMS integration' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'lab', title: 'Lab', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'lab', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['lab']);
      return res.json({ slug: 'lab', items: r.rows || [] });
    }
    return res.json({ slug: 'lab', items: [] });
  } catch (e) {
    return res.json({ slug: 'lab', items: [], error: (e && e.message) || 'Server error' });
  }
});
