...
// Route: /api/gap-operator
// Feature: Operator
// Description: Operator training simulator
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Operator. Operator training simulator' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'operator', title: 'Operator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'operator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['operator']);
      return res.json({ slug: 'operator', items: r.rows || [] });
    }
    return res.json({ slug: 'operator', items: [] });
  } catch (e) {
    return res.json({ slug: 'operator', items: [], error: (e && e.message) || 'Server error' });
  }
});
