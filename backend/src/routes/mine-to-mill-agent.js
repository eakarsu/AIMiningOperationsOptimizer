// Auto-generated feature route: mine-to-mill-agent
// Domain: mine-to-mill integration agent — Capability: coordinating with downstream processors
// Mounted under: /api/mine-to-mill-agent
...
// POST /api/mine-to-mill-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a mine-to-mill integration agent. Your task is coordinating with downstream processors. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/mine-to-mill-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/mine-to-mill-agent/:id — fetch one
...
// DELETE /api/mine-to-mill-agent/:id — discard a run
