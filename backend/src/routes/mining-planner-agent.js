// Auto-generated feature route: mining-planner-agent
// Domain: agentic mining planner — Capability: balancing recovery, safety, and cost across the mine plan
// Mounted under: /api/mining-planner-agent
...
// POST /api/mining-planner-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic mining planner. Your task is balancing recovery, safety, and cost across the mine plan. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/mining-planner-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/mining-planner-agent/:id — fetch one
...
// DELETE /api/mining-planner-agent/:id — discard a run
