// Auto-generated feature route: safety-anomaly-stream
// Domain: mining safety streamer — Capability: streaming anomaly detection across equipment + crew + environment
// Mounted under: /api/safety-anomaly-stream
...
// POST /api/safety-anomaly-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a mining safety streamer. Your task is streaming anomaly detection across equipment + crew + environment. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/safety-anomaly-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/safety-anomaly-stream/:id — fetch one
...
// DELETE /api/safety-anomaly-stream/:id — discard a run
